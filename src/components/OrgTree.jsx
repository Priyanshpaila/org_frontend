...
export default function OrgTree({
      ...
      >
        ...
        <div className="p-3 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="truncate mt-0.5 text-[11px] text-slate-400">
  ...
  const nodeTypes = useMemo(
    () => ({ person: PersonNode, vacant: VacantNode }),
    []
  );
  ...
  const resolveManagerId = (u, idSet) => {
    // 1) reportingTo array (ids or populated docs)
    ...
    return null; // ⬅️ no ancestors fallback
  };
...
const { nodes, edges } = useMemo(() => {
  ...
  // Map each user to its (single) chosen manager (if present in dataset)
  // and manager -> children list
  const managerOf = new Map();     // uid -> mid
  const childrenOf = new Map();    // mid -> [uid...]
  for (const u of data) {
  ...
  // Roots are users with no manager present in this dataset
  const roots = ids.filter((uid) => !managerOf.has(uid));

  // Compute depths purely from manager->children using BFS
  const depthOf = new Map();    // uid -> depth
  ...
  // Any orphan/cycle leftovers → depth 0 so they still render
  for (const uid of ids) {
    if (!depthOf.has(uid)) depthOf.set(uid, 0);
  }

  // Group users by computed depth
  ...
  const maxRow = Math.max(
    ...Array.from(groups.values()).map((arr) => arr.length || 0),
    1
  );
    ...
    row.forEach((u, idx) => {
      ...
      nodes.push({
        ...
        data: {
          user: u,
          color: { fill, stroke },
          onClick: vacant ? undefined : onNodeClick,
        },
      });
    });
  }
  ...
  if (showEdges) {
    ...
    // Strictly manager -> report edges
    for (const [uid, mid] of managerOf) {
      pushEdge(mid, uid);
    }
  }
